...
- at `SessionEnd` and `Stop`, the single transcript path supplied by the harness, only when the user has already connected TokensBurned (`Stop` re-reads it at most every 20 minutes; `SessionStart` reads no transcript and only retries pending uploads).
