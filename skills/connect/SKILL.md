...
- Account privacy is stored by the service for the verified GitHub account and inherited on every connected device. Connecting must never reset or reprompt for an existing policy; new accounts start private. Pass `--publish-card` only when the user explicitly asks to change the account to public and acknowledges that totals, harness/provider/model breakdowns, activity heatmaps, rank, and GitHub identity become public.
...
- When history import is approved, state that only aggregate token and request counts, tool/provider/model labels, and activity dates and hours are uploaded; session identifiers stay local. Prompts, replies, tool payloads, source code, repository paths, and transcript files are not uploaded.
- Start background collection with `tokensburned run` only when the user requests it. Do not add proxies or Git synchronization.
