...
- For first-time setup, explain that `connect` opens GitHub authorization, stores one device credential in `~/.burn/credentials.json`, and then offers an optional history import. Run `connect` only after the user asks to connect or install the service.
